...
function isUserLogged(fname, lname, email, pPanel, products) {
    ...
    for (var i = 0; i < li.length; i++) {
       ...
       if(li[i].children[0].classList.contains('standalones')){
        ...
        if (li[i].children[0].classList.contains("danger")) {
            // li[i].removeAttribute('data-arrange');
            $(".product").sort(sort_li).appendTo('#productList');

            function sort_li(a, b) {
                //return $(a).data('arrange') - $(b).data('arrange') || $(b).data('rdays') - $(a).data('rdays'); // jquery version
        ...
        /* var max = Math.max.apply([], $('li:not(.expired)[data-arrange]').map(function() {
           return $(this).data('arrange');
           //console.log($(this).data('arrange'));
           }).get());   // jquery version */
        ...
        //console.log('standalone ' + standAloneLength);
    ...
    if ($('.product').hasClass('standalone')) {
        $('.standalone').insertAfter('#other');
    }
    ...
    if(standAloneLength == 0){
      _('other').style.display = "none";
    }


    if ($('.product').is('.standalone, .expired')) {
        $('.standalone').insertAfter('#others');
    }
...
function noStandAlone(standAloneLengths){

}

function hideOther(hideOtherLabel){

   if(hideOtherLabel== 0){
                     return  _('other').style.display = "none";
                  }else{
                   return _('other').style.display = "block";
                  }
...
var standAloneLength = document.querySelectorAll('.standalone').length;
  ...
  // console.log('sdfdf ' + standAloneLength);
    for (var i = 0; i < li.length; i++) {
        var prodItems = li[i];
    ...
    /*  if(prodItems.classList.contains('expired') && prodItems.classList.contains('standalone')){
         ...
         /*   if(prodItems.classList.contains('standalone') && showExp){
              console.log('test');
              _('others').style.display = "none";
               _('other').setAttribute('style', 'display:none');
            } */
...
function productLists(item) {
    ...
    if (prodWidgetPan.looksGood()) {
        ...
        // document.querySelector('.date').classList.add('danger');
        indicator = 'danger';
    }
    ...
    return '<li class="media product" data-date="' + item.expirationDate + '" data-rdays="' + item.remainingDays + '"data-skuindi="' + skuIndicator + '" data-indi="' + indicator + '" data-psupport="' + item.isPremiumSupport + '" data-sn="' + item.serial + '" data-maxseat="' + item.maxSeatNo + '" data-pid="' + item.pid + '" data-arrange="' + arg + '" onclick="viewProdInfo(this)">' +
